...
- (void)getCurrentProgress {
  ...
  if (self.onVideoProgress) {
    if (gadMediaView.mediaContent != nil &&
        gadMediaView.mediaContent.hasVideoContent &&
        gadMediaView.mediaContent.duration > 0) {
      self.onVideoProgress(@{
        @"duration" : [NSString
            stringWithFormat:@"%f", gadMediaView.mediaContent.duration],
        @"currentTime" : [NSString
            stringWithFormat:@"%f", gadMediaView.mediaContent.currentTime]
      });
    }
  }
}
